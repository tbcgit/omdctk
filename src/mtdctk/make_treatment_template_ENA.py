...
from mtdctk import (DATE, VERSION, ENA_FASTQ_URLS_COLUMNS, TEMPLATE_FINAL_COLUMNS, 
                    MTD_CT_Exception, program_header, check_existence_directory_parameter, 
...
def check_fastqs_uniqueness(metadata_df, column, fastqs_in_directory, color_treatment):
    ...
    if len(fastqs_warnings_more_than_one_match) > 0 or len(fastqs_warnings_zero_matches) > 0:
        ...
        raise MTD_CT_Exception('Error! Some of the Fastqs in the provided directory do not have a unique match with the Metadata Table (zero or more than one match)!')    
