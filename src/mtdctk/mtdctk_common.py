...
FIGLET = (" __  __ _____ ___     ___              _   _",
          "|  \/  |_   _|   \   / __|  _ _ _ __ _| |_(_)___ _ _",
          "| |\/| | | | | |) |  |(_| || | '_/ _` |  _| / _ \ ' \ ",
          "|_|  |_| |_| |___/   \___\_,_|_| \__,_|\__|_\___/_||_|",
          "              _____         _ _   _ _",
          "             |_   _|__  ___| | |_(_) |_",
          "               | |/ _ \/ _ \ | / / |  _|",
...
#Common classes
class MTD_CT_Exception(Exception):
    pass
...
def treat_headers_check(headers_used, headers_check, table, frase0, frase1, color_treatment):
    ...
    if headers_check == False:
        ...
        #Raise exception
        raise MTD_CT_Exception(' '.join(frase))
...
def get_urls_from_ENA_column(metadata_df, column):
    ...
    if all(check_nans):
        raise MTD_CT_Exception('Error! All cells are empty in the provided ENA Download Column!\n Check your metadata file!')
    else:
        #Get column content
...
def get_list_fastqs_in_directory(directory, fastq_pattern):
    ...
    if len(fastq_files) == 0:
        ...
        raise MTD_CT_Exception(''.join([frase0, fastq_pattern, frase1]))
...
def check_fastq_PAIRED_patterns(fastq_pattern, r1_files_pattern, r2_files_pattern):
    ...
    if r1_files_pattern.endswith(fastq_pattern) == False or r2_files_pattern.endswith(fastq_pattern) == False:
        raise MTD_CT_Exception('Error! The provided Fastq Pattern does not match with some of the PAIRED files patterns!\n Check the provided patterns parameters!')
    elif r1_files_pattern == r2_files_pattern:
        raise MTD_CT_Exception('Error! The provided PAIRED files patterns are identical!\n Check the provided patterns parameters!')
    elif r1_files_pattern == fastq_pattern or r2_files_pattern == fastq_pattern:
        raise MTD_CT_Exception('Error! The provided Fastq Pattern is identical to some of the PAIRED files patterns!\n Check the provided patterns parameters!')
...
def check_rename_samples(treatment_df, unique_samples_list, color_treatment):
    ...
    if len(check_frases) > 0:
        ...
        #Raise exception
        raise MTD_CT_Exception(' '.join(final_frase))
