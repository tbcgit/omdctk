...
def download_metadata_ENA_browser(study_accession, outputdir_path, color_treatment):
    ...
    if response.code == 204:
        raise MTD_CT_Exception('Error! ULR has No Content. The most likely explanation is that the provided Study Accession is not in ENA!\n Check the provided --project parameter!')
    else:
...
def download_metadata_mgtoolkit(study_accession, outputdir_path, color_treatment):
    ...
    if meta_table_t.empty:
        raise MTD_CT_Exception('Error! mg-toolkit returned an empty table. The most likely explanation is that the provided Study Accession is not in ENA!\n Check the provided --project parameter!')
    else:
        #Show saved file message
