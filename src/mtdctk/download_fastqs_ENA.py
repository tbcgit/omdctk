...
from mtdctk import (DATE, VERSION, ENA_FASTQ_URLS_COLUMNS, MTD_CT_Exception, 
...
def LINKS_mode(inputfile_path, color_treatment):
    ...
    if len(urls) == 0:
        raise MTD_CT_Exception('Error! All lines are empty in the provided Links TXT file!')
...
def check_urls_domain_and_treatment(inputfile_type):
    ...
    if (not any(url_checks)) and inputfile_type == 'LINKS':
        raise MTD_CT_Exception('Error! None of the provided Links start with a valid domain (http://, https://, ftp://)!')
