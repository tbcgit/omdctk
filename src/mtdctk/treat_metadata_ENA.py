...
from mtdctk import (DATE, VERSION, ENA_FASTQ_URLS_COLUMNS, TEMPLATE_FINAL_COLUMNS, 
                    VALID_FASTQ_TYPES, VALID_TREATMENTS, MTD_CT_Exception, 
...
def check_treatment_fastqs_in_metadata(treatment_df, metadata_df, column, color_treatment):
    ...
    if len(fastqs_warnings_more_than_one_match) > 0 or len(fastqs_warnings_zero_matches) > 0:
        ...
        raise MTD_CT_Exception('Error! Some of the Fastqs in the Treatment Template do not have a unique match with the Metadata Table (zero or more than one match)!')        
