...
from mtdctk import (DATE, VERSION, MTD_CT_Exception, program_header, 
...
def check_ena_metadata_file_extension(ena_table_path):
    ...
    if ena_table_path.endswith('_ENA_metadata.tsv'):
        ...
        raise MTD_CT_Exception('Error! Expected file extension for ENA Metadata Table is "_ENA_metadata.tsv"!')
...
def treat_merge_columns(ena_merge_col_check, publication_merge_col_check):
    ...
    if ena_merge_col_check == True and publication_merge_col_check == True:
        ...
        raise MTD_CT_Exception('Error! The provided Publication Merge Column is not in the Publication Metadata Table!\n Check the provided --publication_merge_column parameter!')
    elif (ena_merge_col_check == False) and (publication_merge_col_check == True):
        raise MTD_CT_Exception('Error! The provided ENA Merge Column is not in the ENA Metadata Table!\n Check the provided --ena_merge_column parameter!')
    else:
        raise MTD_CT_Exception('Error! Both of the provided merge columns are not in their respective metadata tables!\n Check merge columns parameters!')
